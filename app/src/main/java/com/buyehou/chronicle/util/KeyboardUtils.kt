...
object KeyboardUtils {
    ...
    private fun invokeAbove31(activity: Activity, listener: KeyboardHeightListener) {
        activity.window.decorView.setWindowInsetsAnimationCallback(object :
            ...
            override fun onProgress(
                windowInsets: WindowInsets, list: List<WindowInsetsAnimation>
            ): WindowInsets {
                val imeHeight = windowInsets.getInsets(WindowInsetsCompat.Type.ime()).bottom
                val navHeight =
                    windowInsets.getInsets(WindowInsetsCompat.Type.navigationBars()).bottom
                val hasNavigationBar =
                    windowInsets.isVisible(WindowInsetsCompat.Type.navigationBars()) && windowInsets.getInsets(
                        WindowInsetsCompat.Type.navigationBars()
                    ).bottom > 0
                listener.onKeyboardHeightChanged(
                    if (hasNavigationBar) (imeHeight - navHeight).coerceAtLeast(0) else imeHeight
                )
                return windowInsets
            }
        })
    ...
    fun unregisterKeyboardHeightListener(activity: Activity) {
        ...
        val contentView = activity.window.decorView.findViewById<View>(R.id.content) ?: return
